...
#T# trapping the DEBUG signal (see S1_09_Exception_handling.sh) executes a signal handler after every command
trap "echo in debug mode" DEBUG

#T# the first char in PS4 (see S1_14_Interpreter.sh) is repeated for each level of indirection, e.g. for each level in which a command is executed inside another command
